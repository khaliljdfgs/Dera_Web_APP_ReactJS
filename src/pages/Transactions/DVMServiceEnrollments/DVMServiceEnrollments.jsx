...
import { FetchDVMServiceEnrollments, DeleteDVMServiceEnrollment } from './DVMServiceEnrollmentsAxios'
import { APP_TITLE } from '../../../config'
import { ToastValues } from '../../../values'

const DVMServiceEnrollments = () => {
  ...
  const GetData = useCallback(async (props) => {
    ...
    await FetchDVMServiceEnrollments({
      setError,
      setData,
      navigate,
      setIsLoading,
  ...
  useEffect(() => {
    document.title = `DVM Service Enrollment | ${APP_TITLE}`
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: 'availedBy', header: 'Availed By', size: 0 },
    { accessorKey: 'status', header: 'Status', size: 0 },
    { accessorKey: 'availedAt', header: 'Availed At', size: 0 },
  ], [])
              ...
              renderRowActions={({ row, table }) => (
                <Box sx={{ display: 'flex' }}>
                  <Tooltip
                    ...
                    placement='left'
                    title='View'>
                    <IconButton color='primary' onClick={() => navigate(`/Admin/ServiceEnrollmentInfo/${row.original.uid}`)}>
                      <Visibility />
                    </IconButton>
                  </Tooltip>
                  <Tooltip
          ...
          showDeleteDialogue.status
            ...
            <CustomAlertDialogue
              ...
              positiveCallback={async () => {
                await DeleteDVMServiceEnrollment({
...
export default DVMServiceEnrollments
