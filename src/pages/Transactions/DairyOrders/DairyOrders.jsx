...
import { FetchDairyOrders, DeleteDairyOrder } from './DairyOrdersAxios'
...
const DairyOrders = () => {
  ...
  useEffect(() => {
    document.title = `Dairy Orders | ${APP_TITLE}`
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: 'deliveryTime', header: 'Delivery Time', size: 0 },
    { accessorKey: 'placedBy', header: 'Placed By', size: 0 },
    { accessorKey: 'placedOn', header: 'Place On', size: 0 },
              ...
              renderRowActions={({ row, table }) => (
                <Box sx={{ display: 'flex' }}>
                  <Tooltip
                    ...
                    placement='left'
                    title='View'>
                    <IconButton color='primary' onClick={() => navigate(`/Admin/DairyOrderInfo/${row.original.uid}`)}>
                      <Visibility />
                    </IconButton>
                  </Tooltip>
                  <Tooltip
